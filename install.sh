...
obj={
  ...
  "path":path or "/","tls":"tls","sni":sni,"fp":fp or "chrome",
  "alpn":"","allowInsecure":1,"skip-cert-verify":True
}
...
uninstall_programs() {
    ...
    rm -f /usr/local/bin/np /usr/local/bin/nodeprotocol
    rm -rf "$NODE_HOME"
    systemctl daemon-reload >/dev/null 2>&1 || true
    ok "已删除 Xray、Hysteria2、订阅服务和 np 命令"
}
...
uninstall_complete() {
    local c
    echo
    echo "将删除：全部节点、Xray、Hysteria2、Clash 订阅、np 命令"
