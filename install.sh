...
print_node() {
    ...
    mkdir -p "$CLASH_DIR"
    write_clash_yaml "$CLASH_DIR/$tag.yaml" "$tag"
    print_clash_import "$tag" "$clash_name"
}
...
for tag in tags:
    ...
    reality-opts:
      public-key: {m['pbk']}
      short-id: {m['sid']}"""
    elif m.get("clash_item"):
        block = m.get("clash_item")
    else:
        continue
    items.append(block)
    names.append(name)
...
install_vless_reality() {
    meta_exists vless-reality && { warn "已安装 VLESS+Reality+Vision，请先卸载再装"; return 1; }
    local mode dest_in dest sni port uuid sid fp server spx
    mode="$(choose_mode)"
    ...
    if [[ "$mode" == "custom" ]]; then
        sni="$(ask "SNI / serverName" "$sni")"
    fi

    port="$(choose_tcp_port "443" "$mode")"
    uuid="$(gen_uuid)"
    [[ "$mode" == "custom" ]] && uuid="$(ask "UUID" "$uuid")"
    sid="$(rand_hex 8)"
    [[ "$mode" == "custom" ]] && sid="$(ask "shortId（偶数位十六进制）" "$sid")"
    fp="$(rand_fp)"
...
path=sys.argv[1]
conf=json.load(open(path,encoding="utf-8"))
conf["inbounds"]=[i for i in conf.get("inbounds",[]) if i.get("tag")!="vless-reality"]
conf["inbounds"].append(inbound)
json.dump(conf, open(path,"w",encoding="utf-8"), ensure_ascii=False, indent=2)
PY
    ...
    local name="NP-Reality-${port}"
    save_meta vless-reality <<EOF
{
  "tag": "vless-reality",
...
EOF
    ok "安装完成。伪装站 ${dest}  SNI=${sni}"
    print_node vless-reality
    run_speedtest
}

install_vless_xhttp() {
    meta_exists vless-xhttp && { warn "已安装 VLESS+XHTTP+Reality，请先卸载再装"; return 1; }
    local mode dest_in dest sni port uuid sid fp path server spx
    mode="$(choose_mode)"
    prepare_common || return 1
    install_xray
    ensure_xray_base
    ...
    local pref=""
    meta_exists vless-reality || pref="443"
    port="$(choose_tcp_port "$pref" "$mode")"
    uuid="$(gen_uuid)"
    [[ "$mode" == "custom" ]] && uuid="$(ask "UUID" "$uuid")"
    sid="$(rand_hex 8)"
    [[ "$mode" == "custom" ]] && sid="$(ask "shortId" "$sid")"
    fp="$(rand_fp)"
...
path=sys.argv[1]
conf=json.load(open(path,encoding="utf-8"))
conf["inbounds"]=[i for i in conf.get("inbounds",[]) if i.get("tag")!="vless-xhttp"]
conf["inbounds"].append(inbound)
json.dump(conf, open(path,"w",encoding="utf-8"), ensure_ascii=False, indent=2)
PY
    xray_test_restart || die "Xray 启动失败"
    ...
    local name="NP-XHTTP-${port}"
    save_meta vless-xhttp <<EOF
{
  "tag": "vless-xhttp",
...
EOF
    ok "安装完成。伪装站 ${dest}  SNI=${sni}  path=${path}"
    print_node vless-xhttp
    run_speedtest
}

install_hysteria2() {
    meta_exists hysteria2 && { warn "已安装 Hysteria2，请先卸载再装"; return 1; }
    local mode masq_in masq port password obfs server
    mode="$(choose_mode)"
    prepare_common || return 1
    install_hy2_bin
    ...
    [[ "$mode" == "custom" ]] && password="$(ask "连接密码" "$password")"
    obfs="$(rand_alnum 20)"
    [[ "$mode" == "custom" ]] && obfs="$(ask "salamander 混淆密码" "$obfs")"
    server="$(get_public_ip)"
    local bw
    bw="$(detect_hy2_bandwidth)"

    mkdir -p /etc/hysteria
    make_selfsigned "$masq" /etc/hysteria/server.crt /etc/hysteria/server.key
...
EOF
    if id hysteria >/dev/null 2>&1; then
        chown hysteria:hysteria "$HY2_CONF" /etc/hysteria/server.crt /etc/hysteria/server.key 2>/dev/null || true
    fi
    hy2_restart
    open_firewall "$port" udp

    local name="NP-Hy2-${port}"
    save_meta hysteria2 <<EOF
{
  "tag": "hysteria2",
  "protocol": "Hysteria2",
  "name": "$name",
...
EOF
    ok "安装完成。伪装 ${masq}，UDP ${port}，带宽 ${bw}"
    print_node hysteria2
    run_speedtest
}

install_ss2022() {
    meta_exists ss2022 && { warn "已安装 Shadowsocks 2022，请先卸载再装"; return 1; }
    local mode port password method server
    mode="$(choose_mode)"
    prepare_common || return 1
    install_xray
    ensure_xray_base
    ...
    if [[ "$mode" == "custom" ]]; then
        ...
        password="$(ask "密码（Base64）" "$password")"
    fi
    server="$(get_public_ip)"
...
path=sys.argv[1]
conf=json.load(open(path,encoding="utf-8"))
conf["inbounds"]=[i for i in conf.get("inbounds",[]) if i.get("tag")!="ss2022"]
conf["inbounds"].append(inbound)
json.dump(conf, open(path,"w",encoding="utf-8"), ensure_ascii=False, indent=2)
PY
    xray_test_restart || die "Xray 启动失败"
    ...
    local name="NP-SS2022-${port}"
    save_meta ss2022 <<EOF
{
  "tag": "ss2022",
...
install_socks5() {
    meta_exists socks5 && { warn "已安装 SOCKS5，请先卸载再装"; return 1; }
    local mode port user password server
    mode="$(choose_mode)"
    prepare_common || return 1
    install_xray
    ensure_xray_base
...
path=sys.argv[1]
conf=json.load(open(path,encoding="utf-8"))
conf["inbounds"]=[i for i in conf.get("inbounds",[]) if i.get("tag")!="socks5"]
conf["inbounds"].append(inbound)
json.dump(conf, open(path,"w",encoding="utf-8"), ensure_ascii=False, indent=2)
PY
    xray_test_restart || die "Xray 启动失败"
    ...
    local name="NP-SOCKS5-${port}"
    save_meta socks5 <<EOF
{
  "tag": "socks5",
  "protocol": "SOCKS5",
  "name": "$name",
...
install_vmess_reality() {
    meta_exists vmess-reality && { warn "已安装 VMess + Reality，请先卸载再装"; return 1; }
    local mode dest_in dest sni port uuid sid fp server
    mode="$(choose_mode)"
    prepare_common || return 1
    install_xray
    ensure_xray_base
...
path=sys.argv[1]
conf=json.load(open(path,encoding="utf-8"))
conf["inbounds"]=[i for i in conf.get("inbounds",[]) if i.get("tag")!="vmess-reality"]
conf["inbounds"].append(inbound)
json.dump(conf, open(path,"w",encoding="utf-8"), ensure_ascii=False, indent=2)
PY
    xray_test_restart || die "Xray 启动失败"
    ...
    local name="NP-VMess-${port}"
    save_meta vmess-reality <<EOF
{
  "tag": "vmess-reality",
...
uninstall_tag() {
    ...
    case "$tag" in
        ...
        *)
            ...
            close_firewall "$port" tcp
            [[ "$tag" == "ss2022" || "$tag" == "socks5" ]] && close_firewall "$port" udp
            if [[ "${left:-0}" == "0" ]]; then
                systemctl stop xray >/dev/null 2>&1 || true
                info "Xray inbound 已清空，已停止 Xray"
            else
...
menu_uninstall() {
    ...
    i=1
    for t in "${tags[@]}"; do
        echo "  $i) $(load_meta "$META_DIR/$t.json" protocol)"
        i=$((i+1))
    done
    echo "  9) 卸载全部协议（保留 Xray/Hy2 程序）"
    echo "  0) 返回"
    read -rp "请选择: " c
    [[ "$c" == "0" || -z "$c" ]] && return
    if [[ "$c" == "9" ]]; then
