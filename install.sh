...
NP_SELF_URLS=(
    "https://raw.githubusercontent.com/xiajin88/geto/main/install.sh"
    "https://cdn.jsdelivr.net/gh/xiajin88/geto@main/install.sh"
    "https://github.com/xiajin88/geto/raw/main/install.sh"
)
...
_np_realpath() { readlink -f "$1" 2>/dev/null || printf '%s' "$1"; }

# bash <(curl ...) 时 BASH_SOURCE 是已读完的管道，cat 会得到空文件；
# 也不能 cat 自己覆盖自己，否则会把已保存的脚本截成 0 字节。
_np_is_self_script() {
    local f="$1" n
    [[ -f "$f" && -s "$f" ]] || return 1
    n="$(wc -c < "$f" | tr -d '[:space:]')"
    [[ "$n" =~ ^[0-9]+$ ]] && (( n > 20000 )) || return 1
    grep -q '^#!/usr/bin/env bash' "$f" 2>/dev/null || return 1
    grep -q 'NP_VERSION=' "$f" 2>/dev/null || return 1
    grep -q 'main_menu()' "$f" 2>/dev/null || return 1
}

persist_self() {
    mkdir -p "$NODE_HOME"
    local dest="$NODE_HOME/install.sh"
    local src="${BASH_SOURCE[0]:-}"
    local src_real dest_real tmp url

    dest_real="$(_np_realpath "$dest")"
    if [[ -n "$src" && -f "$src" && -s "$src" ]]; then
        src_real="$(_np_realpath "$src")"
        if [[ "$src_real" != "$dest_real" ]] && _np_is_self_script "$src"; then
            cp -f "$src" "$dest"
        fi
    fi

    if ! _np_is_self_script "$dest"; then
        tmp="$(mktemp 2>/dev/null || echo /tmp/np-self.$$)"
        for url in "${NP_SELF_URLS[@]}"; do
            if curl -fsSL --max-time 30 "$url" -o "$tmp" 2>/dev/null && _np_is_self_script "$tmp"; then
                cp -f "$tmp" "$dest"
                break
            fi
        done
    ...
    if _np_is_self_script "$dest"; then
        chmod +x "$dest"
        ln -sf "$dest" /usr/local/bin/geto
        ln -sf "$dest" /usr/local/bin/np
        ln -sf "$dest" /usr/local/bin/nodeprotocol
    else
        warn "未能写入 geto（本地脚本为空且下载失败），请重新执行安装命令"
    fi
}
...
def clash_from_link(link, name, server, port):
    ...
    if scheme == "vmess":
        ...
        if tls:
            ...
            pcs = (o.get("pcs") or o.get("pinnedPeerCertSha256") or "").strip()
            if pcs:
                lines.append("    pin-sha256: %s" % pcs.split(",")[0].strip())
        if o.get("sni"):
...
cert_sha256_hex() {
    local crt="$1" fp
    [[ -f "$crt" ]] || return 1
    fp="$(openssl x509 -noout -fingerprint -sha256 -in "$crt" 2>/dev/null | sed 's/^.*=//;s/://g' | tr 'A-F' 'a-f' | tr -d '[:space:]')"
    [[ "$fp" =~ ^[0-9a-f]{64}$ ]] || return 1
    printf '%s' "$fp"
}

vmess_cert_pcs() {
    local tag="$1" f="$META_DIR/$tag.json" pcs crt
    pcs="$(load_meta "$f" pcs | tr -d '[:space:]')"
    pcs="$(printf '%s' "$pcs" | tr 'A-F' 'a-f')"
    if [[ "$pcs" =~ ^[0-9a-f]{64}$ ]]; then
        printf '%s' "$pcs"
        return 0
    fi
    crt="/usr/local/etc/xray/certs/${tag}.crt"
    [[ -f "$crt" ]] || crt="$NODE_HOME/certs/${tag}.crt"
    cert_sha256_hex "$crt"
}

ensure_vmess_pcs() {
    local tag="$1" f="$META_DIR/$tag.json" pcs
    [[ "$tag" == vmess-ws* ]] || return 0
    [[ -f "$f" ]] || return 0
    pcs="$(vmess_cert_pcs "$tag")"
    [[ -n "$pcs" ]] || return 0
    python3 - "$f" "$pcs" <<'PY'
import json,sys
p,pcs=sys.argv[1],sys.argv[2]
...
xray_chown_cert() {
    ...
    if id "$u" >/dev/null 2>&1; then
        ...
        chown "$u:$g" "$f" 2>/dev/null || chown "$u" "$f" 2>/dev/null || true
    fi
}
...
for tag in tags:
    ...
    if m.get("clash_item") and kind not in ("socks-vless",) and not (
        ...
        or tag.startswith("vmess-ws")
        or tag.startswith("hysteria2")
    ...
    reality-opts:
      public-key: {m['pbk']}
      short-id: {m['sid']}"""
        elif tag.startswith("vmess-ws"):
            pcs = (m.get("pcs") or "").strip()
            pin = f"\n    pin-sha256: {pcs}" if pcs else ""
            block = f"""  - name: {name}
    type: vmess
    server: {server}
    port: {port}
    ...
    udp: true
    tls: true
    skip-cert-verify: true{pin}
    servername: {m.get('sni') or ''}
    client-fingerprint: {m.get('fp') or 'chrome'}
    network: ws
    ws-opts:
      path: {m.get('path') or '/'}
      headers:
        Host: {m.get('host') or m.get('sni') or ''}"""
...
build_share() {
    ...
    case "$tag" in
        ...
        vmess-ws*)
            uuid="$(load_meta "$f" uuid)"; sni="$(load_meta "$f" sni)"
            path="$(load_meta "$f" path)"; fp="$(load_meta "$f" fp)"
            local host pcs
            host="$(load_meta "$f" host)"
            [[ -n "$host" ]] || host="$sni"
            pcs="$(vmess_cert_pcs "$tag")"
            PYTHONIOENCODING=utf-8 python3 - "$uuid" "$server" "$port" "$name" "$sni" "$host" "$path" "$fp" "$pcs" <<'PY'
import json,base64,sys
uuid,server,port,name,sni,host,path,fp,pcs=sys.argv[1:10]
obj={
  "v":"2","ps":name,"add":server,"port":str(port),"id":uuid,
  "aid":"0","scy":"auto","net":"ws","type":"none","host":host,
  "path":path or "/","tls":"tls","sni":sni,"fp":fp or "chrome","alpn":""
}
if pcs:
    obj["pcs"]=pcs
...
if tag.startswith("vless-xhttp"):
    ...
    pcs=(m.get("pcs") or "").strip()
    if pcs:
        print(f"  pin-sha256: {pcs}")
    print(f"  servername: {m.get('sni','')}")
    print(f"  client-fingerprint: {m.get('fp','chrome')}")
    print(f"  network: ws")
    print(f"  ws-opts:")
    print(f"    path: {m.get('path','/')}")
    print(f"    headers:")
    print(f"      Host: {m.get('host') or m.get('sni') or ''}")
elif tag.startswith("hysteria2"):
    print(f"- name: {name}")
...
list_installed_tags() {
    local t f
    shopt -s nullglob
    for t in vless-reality vless-xhttp hysteria2 ss2022 socks5 vmess-ws vmess-reality; do
...
install_vmess_ws() {
    local mode dest_in dest sni fp path ip tag port uuid listen name crt key pcs
    local done_tags=() first=1
    choose_install_ips || return 1
    filter_install_ips vmess-ws || return 1
    mode="$(choose_mode)"
    prepare_common || return 1
    install_xray
    ensure_xray_base
    local oldf oldtag
    shopt -s nullglob
    for oldf in "$META_DIR"/vmess-reality.json "$META_DIR"/vmess-reality-*.json; do
        [[ -f "$oldf" ]] || continue
        oldtag="$(basename "$oldf" .json)"
        warn "正在卸载旧版 VMess Reality：${oldtag}"
        uninstall_tag "$oldtag"
    done

    dest_in=""
    [[ "$mode" == "custom" ]] && dest_in="$(ask "伪装域名 / SNI（留空则随机）" "")"
    dest="$(pick_random_host "$dest_in")"
    sni="$dest"
    [[ -n "$dest" && -n "$sni" ]] || { err "SNI 不能为空"; return 1; }
    [[ "$mode" == "custom" ]] && sni="$(ask "SNI / Host" "$sni")"
    path="$(rand_path)"
    [[ "$mode" == "custom" ]] && path="$(ask "WebSocket path" "$path")"
    [[ "$path" == /* ]] || path="/$path"
    fp="$(rand_fp)"
    [[ "$mode" == "custom" ]] && fp="$(ask "客户端指纹 chrome/firefox/safari/ios/edge" "$fp")"
    ...
    for ip in "${INSTALL_IPS[@]}"; do
        ...
        uuid="$(gen_uuid)"
        crt="/usr/local/etc/xray/certs/${tag}.crt"
        key="/usr/local/etc/xray/certs/${tag}.key"
        make_selfsigned "$sni" "$crt" "$key"
        xray_chown_cert "$crt"
        xray_chown_cert "$key"
        pcs="$(cert_sha256_hex "$crt")"
        info "写入 Xray inbound（${ip}:${port}）..."
        python3 - "$XRAY_CONF" "$tag" "$listen" "$port" "$uuid" "$sni" "$path" "$crt" "$key" <<'PY'
import json,sys
path,tag,listen=sys.argv[1],sys.argv[2],sys.argv[3]
port=int(sys.argv[4]); uuid=sys.argv[5]; sni=sys.argv[6]
wspath=sys.argv[7]; crt=sys.argv[8]; key=sys.argv[9]
inbound={
  "tag": tag,
  "listen": listen,
  "port": port,
...
EOF
        ok "安装完成 ${ip}:${port}  WS ${path}  SNI=${sni}（自签证书，客户端用证书指纹 pcs）"
        done_tags+=("$tag")
    done
    print_nodes_with_sep "${done_tags[@]}"
    run_speedtest
...
uninstall_tag() {
    ...
    rm -f "$f" "$CLASH_DIR/$tag.yaml" \
        "$NODE_HOME/certs/${tag}.crt" "$NODE_HOME/certs/${tag}.key" \
        "/usr/local/etc/xray/certs/${tag}.crt" "/usr/local/etc/xray/certs/${tag}.key"
    ok "已卸载 $tag"
}

uninstall_programs() {
    local port u
    if [[ -f "$NODE_HOME/sub.json" ]]; then
        port="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1],encoding="utf-8")).get("port",""))' "$NODE_HOME/sub.json" 2>/dev/null || true)"
        [[ "$port" =~ ^[0-9]+$ ]] && close_firewall "$port" tcp
    fi
    systemctl stop np-clash-sub >/dev/null 2>&1 || true
    systemctl disable np-clash-sub >/dev/null 2>&1 || true
    rm -f /etc/systemd/system/np-clash-sub.service

    shopt -s nullglob
    for u in /etc/systemd/system/np-hysteria2*.service; do
        [[ -f "$u" ]] || continue
        systemctl stop "$(basename "$u")" >/dev/null 2>&1 || true
        systemctl disable "$(basename "$u")" >/dev/null 2>&1 || true
        rm -f "$u"
    done
    systemctl stop hysteria-server >/dev/null 2>&1 || true
    systemctl disable hysteria-server >/dev/null 2>&1 || true
    ...
    if curl -fsSL --max-time 20 https://github.com/XTLS/Xray-install/raw/main/install-release.sh >/tmp/np-xray-uninst.sh 2>/dev/null; then
        bash /tmp/np-xray-uninst.sh remove >/dev/null 2>&1 || true
        rm -f /tmp/np-xray-uninst.sh
    fi
    ...
    rm -f /etc/sysctl.d/99-nodeprotocol-net.conf
    rm -f /usr/local/bin/geto /usr/local/bin/np /usr/local/bin/nodeprotocol
    rm -rf "$NODE_HOME"
    systemctl daemon-reload >/dev/null 2>&1 || true
    ok "已删除 Xray、Hysteria2、订阅服务和 geto 命令"
}
...
uninstall_complete() {
    local c
    echo
    echo "将删除：全部节点、Xray、Hysteria2、Clash 订阅、geto 命令"
    read -rp "确认完整卸载？输入 y 继续: " c
...
menu_uninstall() {
    ...
    echo "  b) 完整卸载（全部协议 + Xray/Hy2 程序）"
    echo "  0) 返回"
    read -rp "请选择: " c
    [[ "$c" == "0" || -z "$c" ]] && return
    if [[ "$c" == "a" || "$c" == "A" ]]; then
        [[ ${#tags[@]} -eq 0 ]] && { warn "没有可卸载的协议"; return; }
        uninstall_all
        return
    fi
    if [[ "$c" == "b" || "$c" == "B" ]]; then
