...
build_share() {
    ...
    case "$tag" in
        ...
        vmess-ws*)
            ...
            local host; host="$(load_meta "$f" host)"
            [[ -n "$host" ]] || host="$sni"
            PYTHONIOENCODING=utf-8 python3 - "$uuid" "$server" "$port" "$name" "$sni" "$host" "$path" "$fp" <<'PY'
import json,base64,sys
uuid,server,port,name,sni,host,path,fp=sys.argv[1:9]
obj={
...
install_vmess_ws() {
    local mode dest_in dest sni fp path ip tag port uuid listen name crt key
    local done_tags=() first=1
    choose_install_ips || return 1
...
EOF
        ok "安装完成 ${ip}:${port}  WS ${path}  SNI=${sni}（自签证书，客户端请允许不安全）"
        done_tags+=("$tag")
    done
    print_nodes_with_sep "${done_tags[@]}"
    run_speedtest
...
banner() {
    ...
    echo "  6) VMess + WS + TLS                   直连（兼容 3x-ui）"
    echo
    echo "中转"
    echo "  7) 添加中转（粘贴节点链接）"
    echo
