...
def clash_from_link(link, name, server, port):
    ...
    if scheme == "vmess":
        ...
        if tls and str(tls).lower() not in ("none", "off", "false", "0"):
...
build_share() {
    ...
    case "$tag" in
        ...
        hysteria2*)
            ...
            ;;
        vmess-ws*)
            uuid="$(load_meta "$f" uuid)"
            path="$(load_meta "$f" path)"
            local host; host="$(load_meta "$f" host)"
            [[ -n "$host" ]] || host="$(load_meta "$f" sni)"
            PYTHONIOENCODING=utf-8 python3 - "$uuid" "$server" "$port" "$name" "$host" "$path" <<'PY'
import json,base64,sys
uuid,server,port,name,host,path=sys.argv[1:7]
obj={
  "v":"2","ps":name,"add":server,"port":str(port),"id":uuid,
  "aid":"0","scy":"auto","net":"ws","type":"none","host":host,
  "path":path or "/","tls":"none"
}
...
install_vmess_ws() {
    local mode dest_in dest sni path ip tag port uuid listen name
    local done_tags=() first=1
    choose_install_ips || return 1
    local oldf oldtag
    shopt -s nullglob
    for ip in "${INSTALL_IPS[@]}"; do
        ...
        [[ -f "$META_DIR/$tag.json" ]] || continue
        warn "正在替换已有 VMess：${tag}"
        uninstall_tag "$tag"
    done
    ...
    dest_in=""
    [[ "$mode" == "custom" ]] && dest_in="$(ask "伪装 Host（留空则随机）" "")"
    dest="$(pick_random_host "$dest_in")"
    sni="$dest"
    [[ -n "$dest" && -n "$sni" ]] || { err "伪装 Host 不能为空"; return 1; }
    [[ "$mode" == "custom" ]] && sni="$(ask "WebSocket Host" "$sni")"
    path="$(rand_path)"
    ...
    for ip in "${INSTALL_IPS[@]}"; do
        ...
        uuid="$(gen_uuid)"
        info "写入 Xray inbound（${ip}:${port}）..."
        python3 - "$XRAY_CONF" "$tag" "$listen" "$port" "$uuid" "$path" <<'PY'
import json,sys
path,tag,listen=sys.argv[1],sys.argv[2],sys.argv[3]
port=int(sys.argv[4]); uuid=sys.argv[5]; wspath=sys.argv[6]
inbound={
  "tag": tag,
  "listen": listen,
  "port": port,
...
EOF
        ok "安装完成 ${ip}:${port}  WS ${path}  Host=${sni}"
        done_tags+=("$tag")
    done
    print_nodes_with_sep "${done_tags[@]}"
    run_speedtest
