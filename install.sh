...
install_vmess_ws() {
    ...
    for ip in "${INSTALL_IPS[@]}"; do
        ...
        uuid="$(gen_uuid)"
        crt="$NODE_HOME/certs/${tag}.crt"
        key="$NODE_HOME/certs/${tag}.key"
...
uninstall_tag() {
    ...
    rm -f "$f" "$CLASH_DIR/$tag.yaml" "$NODE_HOME/certs/${tag}.crt" "$NODE_HOME/certs/${tag}.key"
    ok "已卸载 $tag"
}
